...
public class BinaryTree implements Dictionary {
    ...
    public Entry remove(Object key) {
        ...
        if (node.leftChild == null && node.rightChild == null) {
            if ((Integer) node.entry.key < (Integer) node.parent.entry.key) {
            ...
            if (node.parent == null) {
                ...
                root.parent = null;
            } else if ((Integer) node.entry.key < (Integer) node.parent.entry.key) {
            ...
            BinaryTreeNode max = node.rightChild;
            while (max.leftChild != null) {
                max = max.leftChild;
            }
            //Deal with max's child. Since max is auto the most left to the right of node, no left children
            if (max.rightChild != null) {
                max.parent.leftChild = max.rightChild;
                max.rightChild.parent = max.parent;
            }
            //Set Max to replace node
            if (max.leftChild != node.leftChild) {
                max.leftChild = node.leftChild;
                max.leftChild.parent = max;
            }
            if (max != node.rightChild) {                       //Possibility that node's right child was max
            ...
            if (node == root) {
                ...
                node.parent.rightChild = max;                   //It searchs to the right of node, so it must replace node's right child
            }
        }
        size--;
        return entry;
